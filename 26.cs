...
class StringByWordLengths : IEquatable<StringByWordLengths>
{
    public string Original { get; }
    private readonly List<int> _wordLengths;
    public StringByWordLengths(string line)
    {
        Original = line;
        string[] words = line.Split(new[] { ' ' }, StringSplitOptions.RemoveEmptyEntries);
        _wordLengths = words.Select(w => w.Length).OrderBy(l => l).ToList();
    }
    public bool Equals(StringByWordLengths other)
    {
        if (other == null) return false;
        if (_wordLengths.Count != other._wordLengths.Count) return false;
        for (int i = 0; i < _wordLengths.Count; i++)
            if (_wordLengths[i] != other._wordLengths[i]) return false;
        return true;
    }
    public override bool Equals(object obj) => Equals(obj as StringByWordLengths);
    public override int GetHashCode()
    {
        int hash = 17;
        foreach (var len in _wordLengths)
            hash = hash * 31 + len;
        return hash;
    }
...
class Program26
{
    static void Main()
    {
        ...
        var set = new MyHashSet<StringByWordLengths>();
        using (StreamReader reader = new StreamReader(filePath))
        {
            ...
            while ((line = reader.ReadLine()) != null)
            {
                ...
                set.Add(new StringByWordLengths(line));
            }
        }

        Console.WriteLine("Уникальные строки (сравнение по длинам слов):");
